...
public class v1_12_R1 implements NMS
{
    public static NBTTagCompound getTag(final ItemStack arg0, final boolean create) {
        ...
        try {
            ...
            item = (net.minecraft.server.v1_12_R1.ItemStack)handle.get(arg0);
        }
        catch (Exception e) {
    ...
    public void sendLabel(final Player p, final String message) {
        ...
        final PacketPlayOutChat packet = new PacketPlayOutChat(IChatBaseComponent.ChatSerializer.a("{\"text\": \"" + message + "\"}"), ChatMessageType.GAME_INFO);
        connection.sendPacket((Packet)packet);
    }
    
    public void playParticle(final Location loc, final String ep) {
        if (!ep.equalsIgnoreCase("NONE")) {
    ...
    public boolean isPlayerGliding(final Player player) {
        return ((CraftPlayer)player).getHandle().getFlag(7);
    }
    ...
    public boolean hasParticle(final String ep) {
        return this.getParticle(ep) != null;
    }
